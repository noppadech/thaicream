...
class MY_Controller extends MX_Controller {
    ...
    private function _setup() {
		...
		if(!empty($config['progect_id'])){
			//echo $config['progect_id'];
			$key = false;
			if(!empty($this->session->userdata('cotton_us')))
				$key = array_search($config['progect_id'],$this->session->userdata('cotton_us'));
		...
		$this->config->load('ci_bootstrap_menu', TRUE);
		$ci_bootstrap_menu = $this->config->item('ci_bootstrap_menu');
       ...
       // $this->mMenu = empty($config['menu']) ? array() : $config['menu'];
		...
		if(!empty($ci_bootstrap_menu['ci_bootstrap_menu']['menu'])){
			$this->mMenu = empty($ci_bootstrap_menu['ci_bootstrap_menu']['menu']) ? array() : $ci_bootstrap_menu['ci_bootstrap_menu']['menu'];
		}else{
			$this->mMenu = empty($config['menu']) ? array() : $config['menu'];
		}
    ...
    //Upload Images Only.
    protected function upload_img($field_name, $path, $allowed_types = 'png|jpg|jpeg', $resize_width = 300, $resize_height = 150) {
        $config = array();
        $config['upload']['upload_path'] = $path;
        $config['upload']['allowed_types'] = $allowed_types;
        $this->load->library('upload', $config['upload']);
        $this->upload->initialize($config['upload']);
        if ($this->upload->do_upload($field_name)) {
            $data = $this->upload->data();
            $newname = md5(date('Ymdhis') . md5_file($data['full_path'])) . $data['file_ext'];
            rename($data['full_path'], $data['file_path'] . $newname);
            $new_path = $data['file_path'] . $newname;
            $config['resize']['image_library'] = 'gd2';
            $config['resize']['source_image'] = $new_path;
            $config['resize']['new_image'] = $config['upload']['upload_path'] . $newname;
            $config['resize']['create_thumb'] = TRUE;
            $config['resize']['maintain_ratio'] = TRUE;
            $config['resize']['width'] = $resize_width;
            $config['resize']['height'] = $resize_height;
    ...
    protected function upload_file($field_name, $path, $allowed_types) {
        $config = array();
        $config['upload']['upload_path'] = $path;
        $config['upload']['allowed_types'] = $allowed_types;
        $this->load->library('upload', $config['upload']);
    ...
    protected function upload_file_fda($field_name, $path, $allowed_types) {
        $config = array();
        $config['upload']['upload_path'] = $path;
        $config['upload']['allowed_types'] = $allowed_types;
        $this->load->library('upload', $config['upload']);
