...
class Ion_auth_model extends CI_Model
{
	...
	public function add_to_cotton($cotton_ids, $user_id=false)
	{
		$this->trigger_events('add_to_cotton');

		// if no id was passed use the current users id
		$user_id || $user_id = $this->session->userdata('user_id');

		if(!is_array($cotton_ids))
		{
			$cotton_ids = array($cotton_ids);
		}

		$return = 0;

		// Then insert each into the database
		foreach ($cotton_ids as $group_id)
		{
			if ($this->db->insert('admin_users_groups_module', array( $this->join['groups'] => (float)$group_id, $this->join['users'] => (float)$user_id)))
			{
				if (isset($this->_cache_groups[$group_id])) {
					$group_name = $this->_cache_groups[$group_id];
				}
