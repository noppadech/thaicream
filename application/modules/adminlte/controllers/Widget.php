...
class Widget extends MX_Controller {
    ...
    function box_content_open($style = 'primary', $solid = FALSE) {
        $data = array(
            'style' => empty($style) ? '' : 'box-' . $style,
            'solid' => $solid ? 'box-solid' : '',
        );
        $this->load->view('widget/box_content_open', $data);
    }

    function box_content_close($footer = '') {
        $data = array(
            'footer' => $footer,
        );
        $this->load->view('widget/box_content_close', $data);
    }
    ...
    function dialog_box($url, $class, $style, $target, $message = '', $dialog_style = 'info', $title_message = "Message Box", $btn_submit = "Submit") {
        $data = array(
            'url' => $url,
            'class' => $class,
            'style' => $style,
        ...
        $this->load->view('widget/dialog_box', $data);
    }
