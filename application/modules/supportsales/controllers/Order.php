...
class Order extends Admin_Controller {
	...
	public function create($id=NULL) {
		...
		if($id!=NULL || !empty($this->input->post('order_id',true))){
			if($this->form_validation->run() == FALSE){
				...
				$this->mo_order->order_date = splitDateForm4($this->input->post('order_date',true));
				$date1 = str_replace('-', '/', splitDateForm4($this->input->post('order_date',true)));
				...
				$this->mo_order->sales_id = $this->input->post('sales_id',true);
				...
				//header('Location: '.base_url('supportsales/order/'));
		...
		$this->mViewData['sales_name'] = $this->admin->get_sales_name();
		...
		if($id!=NULL || !empty($this->input->post('order_id',true))){
			...
			if($this->form_validation->run() == FALSE){
				...
				$set_data = array(
								...
								"order_date" => splitDateForm4($this->input->post('order_date',true)),
	...
	public function ajax_check_code_id($code_id=NULL){
		echo $this->mo_order->ajax_check_code_id($code_id);
	}
