...
<div class="row">
                ...
                <table class="table table-striped table-bordered table-autosort" id="datatable_serverside" cellspacing="0" width="100%">
                    ...
                    <tbody>
                        ...
                        foreach ($data_cat as $row) {
                            ...
                            ?>
                                    <tr style="<?PHP if ($date1 >= $row->order_datelate && $row->status == 1) {
                            echo "background-color: rgba(231, 243, 58, 0.45);";
                        } ?>">
                            ...
                            <?PHP echo "ประเภท : ";
                            if ($row->type_order == 0) echo "งานแบรนด์";
                            else echo "งานกิโลกรัม";
                            echo"</br>"; ?>
    ...
    <?php
    $comment_all = modules::run($this->config->item('ci_bootstrap')['link_module'] . '/work_now/get_comment_com', $row->order_id);
    foreach ($comment_all as $coms) {
        ?>
        <?PHP ?>
                                                                <div class="direct-chat-msg right" id="show_com-<?PHP echo $coms->comment_id; ?>">
                                                                  <img class="direct-chat-img" src="<?PHP echo base_url(); ?>assets/uploads/users/<?PHP if (!empty($coms->img)) echo $coms->img;
        else echo "sales-icon-32.png"; ?>" alt="message user image">
    ...
    <script>
        $(document).ready(function () {
            $('#datatable_serverside').DataTable({
                ...
                "serverSide": true,
                "ajax": {
                    "url": "<?php echo base_url('sales/work_now/datatable_now') ?>",
                    "dataType": "json",
                    "type": "POST",
